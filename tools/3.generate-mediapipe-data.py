...
# # Improvement consideration: Use 3D distance to ignore rotation
...
with mp_holistic.Holistic(
    ...
    parser = argparse.ArgumentParser(description="Generate mediapipe landmark data files for front-facing images.")
    parser.add_argument("images", nargs="+", help="List of source image files to process")
    ...
    for idx, file in enumerate(args.images):
        ...
        coord_file = f"./{Path(file).stem}-coords.csv" 
        ...
        length_file = f"./{Path(file).stem}-lengths.csv" 
        ...
        cv2.imwrite(f'./{Path(file).stem}-image.png', annotated_image)
